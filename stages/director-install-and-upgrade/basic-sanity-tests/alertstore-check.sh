...
# Specify test name
test_name=alertstore-check
echo "Test name: $test_name"

sed -e 's/generateName: app-check/generateName: '"$test_name"'/g' \
-e 's/app: app-litmus/app: alertstore-check-litmus/g' \
-e 's/value: test-name/value: '"$test_name"'/g' \
-e 's/value: default /value: default/g' \
-e 's/value: pod-name/value: alertstore/g' oep/litmus/director/common-checks/run_litmus_test.yml \
> oep/litmus/director/common-checks/alertstore_run_litmus_test.yml

cat oep/litmus/director/common-checks/alertstore_run_litmus_test.yml

echo -e "\n"
# Run common health check litmus job
kubectl create -f oep/litmus/director/common-checks/alertstore_run_litmus_test.yml
