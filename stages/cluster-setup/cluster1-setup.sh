...
then
  ...
  sshpass -p $pass ssh -o StrictHostKeyChecking=no $user@$ip -p $port 'git clone -b test-dop-url-as-staging-kubera-saas https://github.com/mayadata-io/oep-e2e-konvoy.git'
  #####################################
  ##          Prerequisites          ##
  #####################################
