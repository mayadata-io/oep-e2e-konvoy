...
echo -e "\n************************ Running GUI dashboard test ***********************************\n"
sshpass -p $pass ssh -o StrictHostKeyChecking=no $user@$ip -p $port 'cd oep-e2e-konvoy && bash stages/director-gui/gui-dashboard.sh node '"'$CI_PROJECT_NAME'"' '"'$CI_PIPELINE_ID'"''
}

node() {

bash utils/pooling jobname:selenium-grid-deploy
bash utils/e2e-cr jobname:gui-dashboard jobphase:Running
...
bash utils/e2e-cr jobname:gui-dashboard jobphase:Completed
}
